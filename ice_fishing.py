...
# Limites horizontais do anzol
HOOK_X_MIN = 40
HOOK_X_MAX = SCREEN_WIDTH - 40
...
def create_penguin_surface():
    ...
    # Vareta de pesca (braço) - mais longa
    pygame.draw.line(surf, (100, 70, 30), (44, 40), (59, 2), 4)
    return surf


def create_hook_surface():
    """
    Cria a superfície do anzol com espaço suficiente para não cortar bordas.
    O anzol é composto por: haste vertical + arco inferior + ponta afiada.
    """
    # Canvas maior para garantir que o arco e a ponta não sejam cortados
    surf = pygame.Surface((28, 42), pygame.SRCALPHA)

    # Haste vertical (topo ao centro)
    pygame.draw.line(surf, (180, 180, 190), (14, 0), (14, 22), 3)

    # Arco do anzol — pygame.draw.arc usa bounding rect.
    # Deixamos margem de 2px em todos os lados para não clipar.
    arc_rect = pygame.Rect(2, 16, 22, 20)
    pygame.draw.arc(surf, (180, 180, 190), arc_rect, math.pi, 2 * math.pi, 3)

    # Ponta afiada (desce a partir da extremidade direita do arco)
    # extremidade direita do arco: x = 2+22 = 24, y_center = 16+10 = 26
    pygame.draw.line(surf, (200, 200, 210), (24, 26), (24, 38), 3)

    # Farpinha da ponta (sinaliza que é um anzol)
    pygame.draw.line(surf, (200, 200, 210), (24, 38), (18, 34), 2)

    return surf


def create_reel_surface(radius=14):
    """
    Cria a superfície do carretel de pesca.
    O carretel tem marcas radiais (raios) que tornam a rotação visível.

    ROTAÇÃO — T2 (auxiliar)
    ────────────────────────
    Esta superfície é rotacionada a cada frame com pygame.transform.rotate()
    dentro de Fisherman.draw_reel(), fazendo os raios girarem visivelmente
    enquanto o anzol sobe ou desce.
    """
    size = radius * 2 + 4
    surf = pygame.Surface((size, size), pygame.SRCALPHA)
    cx = cy = size // 2

    # Corpo externo do carretel (aro cinza escuro)
    pygame.draw.circle(surf, (80, 80, 90), (cx, cy), radius)
    # Aro interno (cilindro onde a linha enrola)
    pygame.draw.circle(surf, (120, 120, 130), (cx, cy), radius - 3)
    # Centro do eixo
    pygame.draw.circle(surf, (60, 60, 70), (cx, cy), radius - 8)
    pygame.draw.circle(surf, (200, 200, 210), (cx, cy), 3)

    # Raios — 6 linhas radiais que tornam a rotação perceptível
    num_spokes = 6
    for i in range(num_spokes):
        angle_rad = math.radians(i * (360 / num_spokes))
        x_end = cx + int((radius - 4) * math.cos(angle_rad))
        y_end = cy + int((radius - 4) * math.sin(angle_rad))
        pygame.draw.line(surf, (170, 170, 180), (cx, cy), (x_end, y_end), 2)

    # Borda do aro (contorno)
    pygame.draw.circle(surf, (50, 50, 60), (cx, cy), radius, 2)
...
def create_boot_surface():
    """Cria a superfície de uma bota velha (obstáculo)."""
    surf = pygame.Surface((54, 40), pygame.SRCALPHA)

    # Cano da bota
    pygame.draw.rect(surf, (70, 45, 30), (8, 4, 18, 20), border_radius=4)
    pygame.draw.rect(surf, (100, 70, 48), (10, 6, 14, 14), border_radius=3)

    # Pé da bota
    pygame.draw.rect(surf, (75, 50, 34), (6, 20, 34, 14), border_radius=6)
    pygame.draw.ellipse(surf, (65, 42, 28), (28, 20, 22, 14))

    # Solado
    pygame.draw.rect(surf, (30, 25, 20), (6, 32, 44, 5), border_radius=2)

    # Costura simples
    for x in range(12, 36, 5):
        pygame.draw.line(surf, (130, 100, 70), (x, 23), (x + 2, 23), 1)
...
class Fisherman(pygame.sprite.Sprite):
    ...
    ROTAÇÃO — T2 (carretel)
    ────────────────────────
    O carretel acoplado à vareta acumula um ângulo (reel_angle) que é
    incrementado/decrementado enquanto o anzol se move.
    draw_reel() aplica pygame.transform.rotate(reel_base, reel_angle)
    a cada frame, fazendo os raios do carretel girarem visivelmente.
    """

    REEL_RADIUS = 14   # raio em pixels do carretel
    ...
    def __init__(self):
        ...
        # Superfície base do carretel (sem rotação)
        self.reel_base  = create_reel_surface(self.REEL_RADIUS)
        # Ângulo acumulado de rotação do carretel (graus)
        self.reel_angle = 0.0

        # Posição do carretel na base da vara de pesca.
        # O pinguim tem 60×80px. rect.topleft = (mid_x - 30, ICE_Y - 80).
        # A vara começa em coordenada local (48, 35) — ombro direito do pinguim.
        # Convertendo para offset relativo ao rect.topleft:
        #   offset_x = 48 - 30 = +18   (à direita do centro)
        #   offset_y = 35 - 80 = -45   (45px acima do midbottom)
        # Usamos midbottom como referência para reel_center (ver property abaixo).
        #   offset relativo ao midbottom: x = 48-30 = +18, y = 35-80 = -45
        self._reel_offset = (18, -45)   # base da vara (ombro do pinguim)

    # ------------------------------------------------------------------
    @property
    def reel_center(self):
        """
        Retorna o centro do carretel em coordenadas de tela.
        Calcula a partir do rect.topleft + offset local dentro do sprite
        (base da vara, coordenada local aproximada (48, 35)).
        """
        return (
            self.rect.topleft[0] + 44,   # x local da base da vara
            self.rect.topleft[1] + 40,   # y local da base da vara
        )

    # ------------------------------------------------------------------
    def update_reel(self, moving_down: bool, moving_up: bool, dt: float):
        """
        Incrementa o ângulo do carretel conforme o movimento do anzol.

        ROTAÇÃO — T2 (carretel)
        ────────────────────────
        • Anzol desce → linha sai → carretel gira no sentido horário  (−)
        • Anzol sobe  → linha entra → carretel gira no sentido anti-horário (+)
        • Velocidade angular proporcional à velocidade do anzol.
        """
        REEL_SPEED = 200.0   # graus por segundo

        if moving_down:
            self.reel_angle -= REEL_SPEED * dt   # sentido horário
        elif moving_up:
            self.reel_angle += REEL_SPEED * dt   # sentido anti-horário
        # Mantém o ângulo em [0, 360) para não acumular valores enormes
        self.reel_angle %= 360.0
    ...
    # ------------------------------------------------------------------
    def draw_reel(self, surface):
        """
        Desenha o carretel com ROTAÇÃO aplicada via pygame.transform.rotate().

        ROTAÇÃO — T2 (carretel)
        ────────────────────────
        pygame.transform.rotate(surface, angle) cria uma nova superfície
        rotacionada. Usamos sempre a imagem base (reel_base) para evitar
        degradação acumulada de qualidade.
        Após rotacionar, o rect é recentrado para manter o carretel fixo
        visualmente (o Pygame expande o bounding box ao rotacionar).
        """
        # ← ROTAÇÃO (T2): rotaciona a imagem base pelo ângulo acumulado
        rotated = pygame.transform.rotate(self.reel_base, self.reel_angle)

        # Recentra o rect no ponto fixo do carretel
        reel_rect = rotated.get_rect(center=self.reel_center)
        surface.blit(rotated, reel_rect)
...
class Hook(pygame.sprite.Sprite):
    ...
    def __init__(self, anchor_x, fisherman):
        ...
        # Ângulo atual de rotação do anzol (graus) — oscilação na água
        self.angle       = 0.0
    ...
    def update(self, keys, dt):
        ...
        self.pos_x = max(HOOK_X_MIN, min(HOOK_X_MAX, self.pos_x + dx))
        ...
        # ← ROTAÇÃO (T2): aciona a rotação do carretel no pinguim
        self.fisherman.update_reel(self.moving_down, self.moving_up, dt)
    ...
    def draw(self, surface):
        ...
        """
        # --- Linha de pesca (do carretel, pelo buraco, até o anzol) ---
        # A linha parte do carretel (base da vara), desce até o buraco no gelo
        # e continua até o anzol na água.
        reel_pos = self.fisherman.reel_center
        hole_pos = (self.anchor_x, ICE_SURFACE_Y)
        end      = (self.rect.centerx, self.rect.centery)
        pygame.draw.line(surface, COLOR_HOOK_LINE, reel_pos, hole_pos, 2)
        pygame.draw.line(surface, COLOR_HOOK_LINE, hole_pos, end, 2)
...
class Fish(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        # A imagem base do peixe aponta para a esquerda.
        # Se nascer indo para a direita (direction = 1), espelha no início.
        if self.direction == 1:
    ...
    def _apply_flip(self):
        ...
        if self.direction == 1:
            # Nadar para a direita → imagem espelhada (flip_x=True)
            # REFLEXÃO: T4
            self.image = pygame.transform.flip(scaled_base, True, False)
        else:
            # Nadar para a esquerda → imagem normal
    ...
    def apply_scale(self, factor=2.0):
        ...
        # Se o peixe está indo para a direita, mantém o flip aplicado
        if self.direction == 1:
...
class Boot(pygame.sprite.Sprite):
    """
    Bota velha (obstáculo).
    Se o anzol encostar nela, o jogador perde o jogo.
    """

    def __init__(self):
        super().__init__()
        self.base_image = create_boot_surface()
        self.image = self.base_image.copy()
        self.rect = self.image.get_rect()
        self.direction = random.choice([-1, 1])
        self.speed = random.uniform(0.8, 1.6)
        self.time_acc = random.uniform(0, math.pi * 2)

        # Nasce fora da tela e entra atravessando o mapa
        self._respawn_from_edge(first_spawn=True)

    def _respawn_from_edge(self, first_spawn=False):
        """Reposiciona a bota na borda oposta com nova altura na água."""
        if not first_spawn:
            # Ao reaparecer, pode inverter direção para variar o fluxo
            self.direction *= random.choice([-1, 1])

        if self.direction == 1:
            # Vem da esquerda para a direita
            self.rect.x = -self.rect.width - random.randint(10, 80)
        else:
            # Vem da direita para a esquerda
            self.rect.x = SCREEN_WIDTH + random.randint(10, 80)

        self.origin_y = float(random.randint(ICE_SURFACE_Y + 65, SCREEN_HEIGHT - 60))
        self.rect.centery = int(self.origin_y)
        self.speed = random.uniform(0.9, 1.8)

    def update(self, dt):
        """Move lentamente na horizontal e oscila na vertical."""
        self.time_acc += dt * 1.8
        self.rect.x += self.direction * self.speed
        self.rect.centery = int(self.origin_y + 8 * math.sin(self.time_acc))

        # Atravessa a tela e reaparece na borda oposta com nova altura
        if self.direction == 1 and self.rect.left > SCREEN_WIDTH:
            self._respawn_from_edge()
        elif self.direction == -1 and self.rect.right < 0:
            self._respawn_from_edge()
...
class IceFishingGame:
    ...
    def _init_game(self):
        ...
        self.boot_group = pygame.sprite.Group()

        # Cria o pinguim pescador
        self.fisherman = Fisherman()

        # Cria o anzol (ancorado no centro do pinguim, com referência ao carretel)
    ...
    def _spawn_boot(self):
        boot = Boot()
        self.boot_group.add(boot)
        self.all_sprites.add(boot)
    ...
    def update(self, dt):
        ...
        # --- Colisão: Anzol × Bota (derrota) ---
        self._check_hook_boot_collision()

        # --- Colisão: Anzol × Power-up ---
        self._check_hook_powerup_collision()

        # --- Colisão: Peixe × Power-up ---
        self._check_fish_powerup_collision()

        # --- Reaparecimento de peixes (mantém quantidade mínima) ---
        if len(self.fish_group) < 2:
            self._spawn_fish()

        # --- Reaparecimento de botas (mantém quantidade mínima) ---
        if len(self.boot_group) < 3:
            self._spawn_boot()
    ...
    # ------------------------------------------------------------------
    def _check_hook_powerup_collision(self):
        """Permite ao jogador coletar a estrela com a ponta do anzol."""
        hook_rect = pygame.Rect(
            self.hook.rect.centerx - 8,
            self.hook.rect.centery - 8,
            16, 16
    ...
    # ------------------------------------------------------------------
    def _check_hook_boot_collision(self):
        """Se o anzol pegar a bota, encerra a partida."""
        hook_rect = pygame.Rect(
            self.hook.rect.centerx - 8,
            self.hook.rect.centery - 8,
            16, 16
    ...
    def draw(self):
        ...
        # Efeito de profundidade da água (gradiente manual em faixas - invertido)
        for i in range(8):
        ...
        # --- Bota (obstáculo) ---
        for boot in self.boot_group:
            self.screen.blit(boot.image, boot.rect)

        # --- Pinguim pescador ---
        self.fisherman.draw(self.screen)

        # --- Carretel do pinguim (ROTAÇÃO — T2) ---
        # O carretel gira visivelmente enquanto o anzol sobe ou desce.
        self.fisherman.draw_reel(self.screen)

        # --- Anzol (linha completa carretel→buraco→anzol + rotação) ---
    ...
    def _draw_ui(self):
        ...
        if self.game_over:
            ...
            game_over_surf = self.font_big.render("FIM DE JOGO", True, COLOR_RED)
            game_over_rect = game_over_surf.get_rect(
                center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 20)
            )
            self.screen.blit(game_over_surf, game_over_rect)

            tip_surf = self.font_small.render("Voce pescou uma bota! Pressione R para reiniciar", True, COLOR_WHITE)
            tip_rect = tip_surf.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 20))
